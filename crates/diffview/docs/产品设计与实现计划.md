...
- Diff 工作区编辑器：Diff 页可打开底部“工作区编辑器”面板（仅当对比目标包含工作区），按文件类型语法高亮；支持应用预览/从磁盘重载/保存到文件并刷新 diff + git status；`Alt+A` 应用、`Cmd/Ctrl+S` 保存。
...
- 性能与体验（基础版）：git/IO/diff 计算移到 `background_executor`；打开 diff 时双侧版本读取并行化；忽略空白/上下文改动采用 120ms 防抖 + 后台重算；虚拟列表 `item_sizes` 缓存减少大向量分配；切换 Split/Inline 复用 diff model 仅重建行列表；**diff 结果 LRU 缓存（不绑定 Split/Inline）+ 邻近文件预取** 降低重复计算与切换等待；可选 `GIT_VIEWER_TIMING=1` 显示加载耗时；提供 Large Diff Demo（`GIT_VIEWER_LARGE_DEMO_LINES`）。
